...
export default async function MyPage() {
  ...
  const { data: orderList } = await getOrdersByUserId(data.user.id);
    ...
    <div>
      ...
      <div className="flex gap-8 rounded-2xl p-4 bg-slate-100">
        ...
        <div className="flex flex-1 flex-col justify-between">
          <div className="mb-8">
            <p className={cn("text-base font-bold", "md:text-2xl")}>{data.user?.user_metadata.username}</p>
            <p className={cn("text-xs text-gray-500 mt-1", "md:text-base")}>{data.user?.email}</p>
            <p className={cn("text-gray-500 mt-1 text-xs", "md:text-sm")}>
