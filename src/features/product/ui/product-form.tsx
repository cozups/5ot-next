...
import { insertProduct, ProductFormState } from "@/features/product/actions";
...
export default function ProductForm({ categories }: ProductFormProps) {
  const [formState, formAction, isPending] = useActionState(insertProduct, initialState);
      ...
      <div className={cn("w-full flex justify-center gap-8 mb-4 flex-col", "md:flex-row md:items-center")}>
        <div className={cn("w-full flex flex-col gap-4", "md:w-1/2")}>
          <div>
            ...
            {<p className="text-red-500 text-sm">{formState?.errors?.errors?.name}</p>}
          </div>
          <div>
          ...
          <div>
            ...
            {<p className="text-red-500 text-sm">{formState?.errors?.errors?.image}</p>}
          </div>
        </div>
